    ...
    try:
        ...
        for p in psutil.process_iter():
            ...
            try:
                ...
                pass
            known_processes[p.pid] = (p, cmd, ppid)
        ...
        while not _poll(pipe, poll_interval=sleep_interval):
            ...
            dead_processes = set()
            # First iterate through known processes to find dead ones.
            for pid in known_processes:
                (p, cmd, ppid) = known_processes[pid]
                if not p.is_running():
                    processes.append((pid, p.create_time(), time.time(), cmd, ppid))
                    dead_processes.add(pid)
            for pid in dead_processes:
                del known_processes[pid]

            # Identify processes created since the last sample
            for p in psutil.process_iter():
                if not p.pid in known_processes:
                    cmd = []
                    try:
                        cmd = p.cmdline()
                    except Exception as e:
...
class SystemResourceMonitor(object):
    ...
    def as_profile(self):
        ...
        def add_marker(
            name_index, start, end, data, category_index=cat_other, precision=None
        ):
        ...
        for phase, v in self.phases.items():
            ...
            add_marker(phase_string_index, v[0], v[1], markerData, cat_phase, 3)
        ...
        for name, start, end, text in self.markers:
            ...
            add_marker(get_string_index(name), start, end, markerData, cat_task, 3)
        if self.events:
        ...
        add_marker(phase_string_index, self.stop_time, now, markerData, cat_phase, 3)
        ...
        add_marker(teardown_string_index, profile_time, now, markerData, cat_task, 3)
